 ...
 *
 */
public class ReactionTableEntry extends BioObject implements ReactionStructureProvider {
     ...
     * @param objects
     */
    public ReactionTableEntry(int index, IReaction reaction, Object[] objects) {
        this(index, reaction, objects, null);
    }
     ...
     * @param colorer
     */
    public ReactionTableEntry(int index, IReaction reaction, Object[] objects, IAtomColorer colorer) {
    ...
    public void draw(Event event) {
        ...
        if (event.index==ReactionTablePage.REACTION_COLUMN){
            if (image!=null)
                event.gc.drawImage(image, event.x, event.y);
            else{
                image=computeReactionImage(event);
                if (image==null){
    ...
    private void drawProperty(Event event) {
        ...
        if (event.index==ReactionTablePage.INDEX_COLUMN){
     ...
     * @param event
     */
    private Image computeReactionImage(Event event) {
        ...
        if (event.widget instanceof Table) {
            ...
            xsize=table.getColumn(ReactionTablePage.REACTION_COLUMN).getWidth();
            xsizeIX=table.getColumn(ReactionTablePage.INDEX_COLUMN).getWidth();
        }
...
//        GeometryTools.scaleMolecule(drawReaction, screenSize, 0.8,coordinates);
    ...
    /**
     * Return the AtomContainer for access via ChemicalReactionProvider
    ...
    public Object getAdapter( Class adapter ) {
        ...
        if (adapter == IMolecule.class){
            CDK10Reaction reaction10=new CDK10Reaction(reaction);
            return reaction10;
        }
